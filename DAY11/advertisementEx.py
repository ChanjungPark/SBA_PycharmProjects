...
가설 공식 = 회귀 방정식 = y = H = w*w + b
회귀선 : 회귀 방정식에 의하여 그려진 선 그래프
머신러닝에서 w를 가중치(기울기)라고 하고, b를 bias라고 부릅니다.
...
최소 자승법
비용함수를 구할 때 제곱을 해서 풀어나가므로 이를 최소 제곱법 또는 최소 자승법이라고 한다
'''
...
plt.ylabel('광고 수익')
plt.ylim([20, 120])
plt.plot(x, y, color='blue', linestyle='solid', marker='o', label='read data')
...
# w를 모른다는 가정 하에 w = 25, 27.5, 30, 35라는 값을 이용하여
# 각각의 비용 함수가 얼마가 되는지 살펴보도록 하겠습니다.
def myfunction(w, x, b = 7.5):
    return w * x + b

b = 10
slope1, slope2, slope3 = 25, 30, 35
# slope1, slope2, slope3 = 10, 30, 45
...
# 위의그래프를 보면 이상적인 w에서 비용 함수의 값이 최소가 됩니다.
# 이 w의 값이 27.5보다 작아지거나 커지면 비용 함수의 값은 커집니다.
# 이것을 그래프로 그려보겠습니다.

# w를 20 이상 35이하의 범위에서 w에 따른 비용 함수를 시각화 해봅니다.
cost = []   # 비용 함수 리스트

for idx in range(2000, 3500, 1):
