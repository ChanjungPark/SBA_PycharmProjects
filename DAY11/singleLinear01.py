...
#   = train_test_split(입력 원본, 출력 원본, test_size=테스트 데이터의 비율)
# 일반적으로 7:3
...
# 모델을 생성하고, 학습을 수행합니다.
# 단계 01 : 모델(작업 공간)을 생성합니다.
model = Sequential()

# 단계 02 : 레이어를 추가합니다.
# Dense : 레이어를 추가하는데 사용되는 클래스
# units=출력값의 크기, input_dim=입력 데이터의 크기, activation='활성화 함수'
# 'linear' : 선형 회귀 분석
...
# epochs : 반복 횟수
# batch_size : 한번에 수행할 데이터의 양
# verbose : 0(진행 결과를 출력하지 않음), 2(epoch당 1번), 1(기본값)
