...
def setScatterGraph(tour_table, visit_table, tourpoint):
    ...
    for onedata in mylist:
        ...
        plt.title(tourpoint + '-' + onedata[1] + ' 상관 관계 분석')
        plt.scatter(x_data, y_data, edgecolors='none', alpha=0.75, c='black')

        # savefig : 해당 이미지를 파일 형식으로 저장하는 옵션
...
def main():
    ...
    # .은 현재 폴더, ..는 상위 폴더
    ########### touristResourceStat(서울특별시 2015~2019).json ###########
    ...
    print(tour_table.head())   # 전체 데이터 중에서 앞 5개만 보여주세요.
    # print(tour_table)
    print('-' * 30)
    ...
    ########### immigrationTouristStat 미국(275)_(2015~2019).json ###########
    ...
    ########### immigrationTouristStat 일본(130)_(2015~2019).json ###########
    ...
    ########### immigrationTouristStat 중국(112)_(2015~2019).json ###########
